...
aws configure set aws_access_key_id ${ACCESS_KEY} && aws configure set aws_secret_access_key ${SECRET_KEY} && aws configure set region ${REGION} && aws --endpoint-url=${ENDPOINT} s3api create-bucket --region ${REGION}  --bucket ${BUCKET} --create-bucket-configuration LocationConstraint=${REGION}  && aws --endpoint-url=${ENDPOINT} s3api put-object --bucket ${BUCKET} --key ${OBJECT_KEY} --body ${FILEPATH}
